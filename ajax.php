...
if (isset($_POST['bookingDetails'])) {
    ...
    if ($booking_id != '') {
        $query = "SELECT
        bookings.*, booking_types.type_name 
          FROM bookings 
          LEFT JOIN booking_types ON bookings.booking_type_id = booking_types.id  WHERE booking_id = '$booking_id'";
        $result = mysqli_query($connection, $query);

        if ($result && mysqli_num_rows($result) > 0) {
            $booking = mysqli_fetch_assoc($result);
            $response['done'] = true;
            $response['customer_name'] = $booking['customer_name'];
            $response['contact_no'] = $booking['contact_no'];
            $response['email'] = $booking['email'];
            $response['id_card'] = $booking['id_card'];
            $response['address'] = $booking['address'];
            $response['checkin_date'] = $booking['checkin_date'];
            $response['checkout_date'] = $booking['checkout_date'];
            $response['rooms'] = $booking['rooms'];
            $response['guests'] = $booking['guests'];
            $response['booking_date'] = $booking['booking_date'];
            $response['note'] = $booking['note'];
            $response['room_type'] = $booking['room_type'];
            $response['type_name'] = $booking['type_name'];
        } else {
            $response['done'] = false;
            $response['data'] = "No booking found";
        }
    } else {
        $response['done'] = false;
        $response['data'] = "Booking ID is required";
    }
...
if (isset($_POST['canceledBookingDetails'])) {
    $booking_id = $_POST['booking_id'];
    $response = array();

    if ($booking_id != '') {
        $query = "SELECT
        cancelled_bookings.*, booking_types.type_name 
          FROM cancelled_bookings 
          LEFT JOIN booking_types ON cancelled_bookings.booking_type_id = booking_types.id  WHERE booking_id = '$booking_id'";
        $result = mysqli_query($connection, $query);
...
if (isset($_POST['cancel_booking'])) {
    ...
    if ($booking) {
        ...
        $insertQuery = "INSERT INTO cancelled_bookings (booking_id, user_id, customer_name, contact_no, email, id_card, address, checkin_date, checkout_date, rooms, guests, booking_date, note,booking_type_id,room_type)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?)";
...
if (isset($_POST['complete_booking'])) {
    ...
    $query = "UPDATE booking SET booking_status = '2' WHERE booking_id = '$booking_id'";
    $result = mysqli_query($connection, $query);

    if ($result) {
        echo "Booking completed successfully.";
    } else {
        echo "Error completing booking.";
    }
}
...
if (isset($_POST['canceled_delete_booking'])) {
    $booking_id = $_POST['booking_id'];

    // Delete booking
    $query = "DELETE FROM cancelled_bookings WHERE booking_id = '$booking_id'";
    $result = mysqli_query($connection, $query);
...
if (isset($_POST['clear_all_canceled_booking'])) {
    // 
    $query = "DELETE FROM cancelled_bookings";
    $result = mysqli_query($connection, $query);

    if ($result) {
        echo "All cancelled bookings deleted successfully.";
    } else {
        echo "Error deleting cancelled bookings: " . mysqli_error($connection);
    }
}


if (isset($_POST['clear_all_booking'])) {
    // 
    $query = "DELETE FROM bookings WHERE checkout_date < CURDATE() ";
    $result = mysqli_query($connection, $query);

    if ($result) {
        echo "All  bookings deleted successfully.";
    } else {
        echo "Error deleting  bookings: " . mysqli_error($connection);
    }
}


if (isset($_POST['active_booking'])) {
    $booking_id = $_POST['booking_id'];

    $query = "UPDATE booking SET booking_status = '0' WHERE booking_id = '$booking_id'";
    $result = mysqli_query($connection, $query);
...
if (isset($_POST['get_rooms_by_type'])) {

    $room_type = $_POST['room_type0'];
    $checkin_date = $_POST['checkin_date0'];
    $checkout_date = $_POST['checkout_date0'];
    $query = "
    SELECT room.*
    FROM room
    LEFT JOIN roombooking ON room.room_id = roombooking.room_id
    LEFT JOIN booking ON roombooking.booking_id = booking.booking_id
    WHERE room.room_type_id = '$room_type'
    AND (booking.check_in IS NULL OR booking.check_out IS NULL OR NOT (
        ('$checkin_date' < booking.check_out) AND ('$checkout_date' > booking.check_in)
    ))
";
    
    //$query = "SELECT * FROM room WHERE room_type_id = '$room_type'";
    $result = mysqli_query($connection, $query);

    $rooms = '<option selected disabled>Select Room Name</option>';
    if (mysqli_num_rows($result) > 0) {
        while ($row = mysqli_fetch_assoc($result)) {
            $rooms .= '<option value="'.$row['room_id'].'">'.$row['room_name'].'</option>';
        }
    }
    echo $rooms;
}
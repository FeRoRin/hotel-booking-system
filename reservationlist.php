...
$query = "SELECT bookings.*, booking_types.type_name 
          FROM bookings 
    ...
    <div class="row">
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">List of Bookings
                <form method="post" action="" style="display:inline;">
    <!--button type="submit" name="download_pdf" class="btn btn-secondary  pull-right">Download PDF</button-->
</form>
                <!--button class="btn btn-secondary pull-right" style="border-radius:0%" data-toggle="modal" data-target="#addRoom">Add Booking</button-->
                </div>
            ...
            <?php if (count($bookings) > 0): ?>
                <?php foreach ($bookings as $booking): ?>
                    ...
                    <td><?php echo $booking['booking_date']; ?></td>
                        ...
                        <td><button class="btn btn-warning btn-sm cancel-booking" data-booking-id="<?php echo $booking['booking_id']; ?>">Cancel</button></td>
...
$(document).ready(function() {
    $('.booking-link').click(function(e) {
        ...
        $.ajax({
            url: 'ajax.php',
            method: 'POST',
            data: { bookingDetails: true, booking_id: bookingId },
            dataType: 'json',
    ...
    $(document).on('click', '.cancel-booking', function(e) {
        ...
        if (confirm('Are you sure you want to cancel this booking?')) {
            $.ajax({
                url: 'ajax.php',
                method: 'POST',
                data: { cancel_booking: true, booking_id: bookingId },
                success: function(response) {
                    alert(response);
                    window.location.href = "index.php?reservationlist";
                    //header("Refresh:0");
...
/*  $(document).ready(function() {
    ...
    e.preventDefault();
    var bookingId = $(this).data('booking-id');
    window.location.href = 'editpage.php?booking_id=' + bookingId;
});
    ...
    $('.cancel-booking').click(function(e) {
        ...
        if (confirm('Are you sure you want to cancel this booking?')) {
            ...
            $.ajax({
                ...
                success: function(response) {
                    alert(response);
                    // Reload page or update booking list as needed
                    location.reload();
                },
                error: function(xhr, status, error) {
                    console.error(xhr.responseText);
    ...
    $('.active-booking').click(function(e) {
        e.preventDefault();
        var bookingId = $(this).data('booking-id');
        if (confirm('Are you sure you want to active this booking?')) {
            // Perform AJAX request to cancel booking
            $.ajax({
                url: 'ajax.php',
                method: 'POST',
                data: { active_booking: true, booking_id: bookingId },
                success: function(response) {
                    alert(response);
                    // Reload page or update booking list as needed
                    location.reload();
                ...
                error: function(xhr, status, error) {
                    console.error(xhr.responseText);
                    alert('Error activing booking.');
                }
            });
        }
    });

    $('.complete-booking').click(function(e) {
        e.preventDefault();
        var bookingId = $(this).data('booking-id');
        if (confirm('Are you sure you want to complete this booking?')) {
            // Perform AJAX request to complete booking
            $.ajax({
                url: 'ajax.php',
                method: 'POST',
                data: { complete_booking: true, booking_id: bookingId },
                success: function(response) {
                    alert(response);
                    // Reload page or update booking list as needed
                    location.reload();
